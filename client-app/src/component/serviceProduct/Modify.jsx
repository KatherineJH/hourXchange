...
function Modify() {
    ...
    useEffect(() => {
        ...
        getRead(id).then(response => {
            ...
            setUpdateData({...response.data, ["categoryId"]: response.data.category.id, ["ownerId"]: response.data.owner.id});
        }).catch(error => console.log(error))
        ...
        <form>
            ...
            <div>
                ...
                <input type="text" id="title" name="title"
                       value={updateData.title} onChange={handleChange}
                />
            </div>
            <div>
                <label htmlFor="description">설명</label>
                <textarea id="description" name="description"
                          value={updateData.description} onChange={handleChange}
                />
            </div>
            <div>
                <label htmlFor="hours">시간(비용)</label>
                <input type="number" id="hours" name="hours"
                       value={updateData.hours} onChange={handleChange}
                />
            </div>
            <div>
                <label htmlFor="startedAt"></label>
                <input type="datetime-local" id="startedAt" name="startedAt"
                       value={updateData.startedAt} onChange={handleChange}
                />
            </div>
            <div>
                <label htmlFor="endAt"></label>
                <input type="datetime-local" id="endAt" name="endAt"
                       value={updateData.endAt} onChange={handleChange}
                />
            </div>
            <div>
                <label htmlFor="categoryId"></label>
                <select id="categoryId" name="categoryId"
                        value={updateData.categoryId} onChange={handleChange}
                >
            ...
            {updateData.images.map((item, index) => (
                <div key={index}>
                    ...
                    <input type="text" id={"image"+index} name={"image"+index}
                           value={item} onChange={e => handleImageChange(index, e.target.value)}
                    />
                </div>
            ))}
            ...
            <div>
                ...
                <select id="providerType" name="providerType"
                        value={updateData.providerType} onChange={handleChange}
                >
