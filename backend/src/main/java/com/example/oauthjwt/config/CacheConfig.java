...
public class CacheConfig {
    ...
    public RedisCacheManager cacheManager(RedisConnectionFactory connectionFactory) {
        ...
        ObjectMapper mapper = new ObjectMapper().registerModule(new JavaTimeModule()) // LocalDate/LocalDateTime 지원
        ...
        // cacheConfigs.put("productFindAll",
        // defaultConfig.entryTtl(Duration.ofMinutes(1)));

        // 필요하다면 다른 캐시들도 여기에 추가 가능

        // 6) RedisCacheManager 빌드
        return RedisCacheManager.builder(connectionFactory).cacheDefaults(defaultConfig) // 나머지 캐시는 기본 설정 사용
