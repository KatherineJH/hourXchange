...
public class ElasticSearchService {
  ...
  public Page<ProductResponse> searchProducts(String keyword, Pageable pageable) { // 엘라스틱서치는 점수와 하이라이팅만 실 데이터는 db에서
    try {
      SearchResponse<ProductDocument> response = client.search(s -> s.index("product_index")
  ...
  public PageResult<BoardDocument> searchBoards(String keyword, int page, int size) {
    try {
      ...
      // results.size());
      // return results;
      List<BoardDocument> hits = response.hits().hits().stream().map(Hit::source).filter(Objects::nonNull).toList();
  ...
  public List<String> autocomplete(String prefix, String index) {
    try {
      SearchResponse<Void> response = client
          .search(s -> s.index(index).suggest(sug -> sug.suggesters("completion_suggest", sugField -> sugField
              .prefix(prefix.toLowerCase()).completion(c -> c.field("suggest").size(10).fuzzy(f -> f.fuzziness("2")) // 편집
                                                                                                                     // 거리
                                                                                                                     // 2
              ))), Void.class);
