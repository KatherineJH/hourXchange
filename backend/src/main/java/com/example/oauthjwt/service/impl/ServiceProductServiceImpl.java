...
import com.example.oauthjwt.dto.CategoryResDTO;
...
public class ServiceProductServiceImpl implements ServiceProductService {
    ...
    public ServiceProductResDTO save(ServiceProductReqDTO serviceProductReqDTO) {
        ...
        Category category = categoryRepository.findById(serviceProductReqDTO.getCategoryId()).get(); // 카테고리 조회

        ServiceProduct serviceProduct = ServiceProduct.builder() // 저장할 객체 생성
                ...
                .owner(owner)
                .category(category)
                .providerType(ProviderType.valueOf(serviceProductReqDTO.getProviderType().toUpperCase()))
                .build();

        for(String url : serviceProductReqDTO.getImages()) { // 이미지 url list 등록
            SPImage spImage = SPImage.builder()
                    .imgUrl(url)
