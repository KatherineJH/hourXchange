...
public class ScheduleServiceImpl {
  ...
  Map<String, String[]> areaCenters = Map.ofEntries(Map.entry("0101", new String[]{"37.5665", "126.9780"}), // 서울
  ...
  // count++;
  // if (response.getBody().getNumOfRows() != response.getBody().getTotalCount())
  // {
  // // 다른경우 마지막 페이지
  // break;
  // }
  ...
  // .queryParam("numOfRows", 500) // 500건씩 저장
  // .queryParam("pageNo", count) // 1페이지부터
  // .queryParam("strDate", LocalDate.now().minusMonths(4).toString()) //
  // 4달
  // 전부터
  ...
  // log.info(xml);
  // try {
  // VollcolectionResponse response = xmlMapper.readValue(xml,
  // VollcolectionResponse.class);
  // log.info(response);
  // List<Product> productList = response.getBody().getItems().stream()
  // .flatMap(item -> {
  // Optional<User> user =
  // userRepository.findByEmail(item.getCentCode());
  // if(!user.isPresent()) {
  ...
  // .orElseThrow(() -> new
  // ResponseStatusException(HttpStatus.NOT_FOUND, "카테고리 정보가 존재하지 않습니다."));
  // ProviderType providerType = ProviderType.VOLUNTEER; // 자원봉사
  //
  // return Stream.of(Product.of(item, user.get(), category,
  ...
  // count++;
  // if (response.getBody().getNumOfRows() !=
