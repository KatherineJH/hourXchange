...
public class PaymentResponse {
    ...
    public static PaymentResponse toDto(Payment payment, User user, PaymentItem paymentItem) {
        return PaymentResponse.builder().id(payment.getId()).impUid(payment.getImpUid())
                .merchantUid(payment.getMerchantUid()).amount(payment.getAmount()).status(payment.getStatus())
                .paidAt(payment.getPaidAt()).payMethod(payment.getPayMethod()).pgProvider(payment.getPgProvider())
                .pgTid(payment.getPgTid()).receiptUrl(payment.getReceiptUrl()).user(UserResponse.toDto(user))
                .paymentItem(PaymentItemResponse.toDto(paymentItem)).build();
    }
}
