...
public class TransactionRepositoryImpl implements TransactionRepositoryCustom {
    ...
    public Page<Transaction> search(TransactionSearchCondition condition, Pageable pageable) {
        ...
        // 실제 데이터 쿼리
        List<Transaction> result = queryFactory.selectFrom(transaction).where(builder).offset(pageable.getOffset())
                .limit(pageable.getPageSize()).fetch();
        // 전체 개수 쿼리
        long total = queryFactory.selectFrom(transaction).where(builder).fetchCount();
