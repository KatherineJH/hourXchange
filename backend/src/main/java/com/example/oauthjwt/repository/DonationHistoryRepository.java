...
public interface DonationHistoryRepository
    ...
    /**
     * 주어진 시점 이후의 기부 기록을 사용자별로 묶어, 기부량 합계가 큰 순서대로 return합니다. 반환 스펙: Object[]{ Long
     * userId, Long totalDonatedHours }
     */
    @Query("""
            ...
            """)
    List<Object[]> findTopDonatorsSince(@Param("since") LocalDateTime since, Pageable pageable);
    ...
    @Query("""
            ...
            """)
    List<Object[]> countByRange(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);
    ...
    @Query("""
            ...
            ORDER BY FUNCTION('DATE', dh.createdAt)
            """)
    List<Object[]> sumAmountByRange(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);
}
