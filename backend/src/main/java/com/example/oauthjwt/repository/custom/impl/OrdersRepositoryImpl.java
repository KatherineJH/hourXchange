...
public class OrdersRepositoryImpl implements OrdersRepositoryCustom {
    ...
    public Page<Orders> search(OrdersSearchCondition condition, Pageable pageable) {
        ...
        // 실제 데이터 쿼리
        List<Orders> result = queryFactory.selectFrom(orders).where(builder).offset(pageable.getOffset())
                .limit(pageable.getPageSize()).fetch();
        // 전체 개수 쿼리
        long total = queryFactory.selectFrom(orders).where(builder).fetchCount();
