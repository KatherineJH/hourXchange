 ...
 * <p>
 * 주요 사용 목적:
 * - 예측 모델에 필요한 데이터를 빠르게 조회하기 위함.
 * <p>
 * 주의사항:
 * - 허용된 테이블 목록에 한해서만 조회가 가능하며, allowedTables에 정의된 테이블만 허용.
 * - 테이블에 저장된 데이터는 날짜 기준으로 내림차순 정렬되어 있어야 함. FILO.
 * - position 파라미터는 "first" 또는 "last" 값만 지원.
 */
@RestController
...
public class CommonController {
    ...
    /**
     * 지정된 테이블에서 첫 번째 또는 마지막 row를 조회.
     * 해당 컨트롤러는 서비스와 레포지토리를 따로 사용하지 않음으로, 예외적으로 예외처리를 컨트롤러에 작성함.
     * @param table    조회할 테이블명 (소문자로 비교되며, allowedTables 내에 있어야 함)
     * @param position "first" 또는 "last" 중 하나의 값으로, ASC 또는 DESC 정렬 여부를 지정
     * @return 해당 테이블의 첫 번째 또는 마지막 row를 Map 형식으로 반환
     * @throws ResponseStatusException 잘못된 테이블명 또는 포지션 값이 들어올 경우 400 Bad Request 응답
     * 예:
     * GET /api/meta/row?table=user&position=first
     */
    @GetMapping("/row")
