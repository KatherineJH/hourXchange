...
public class AuthController {
  ...
  public ResponseEntity<?> login(@RequestBody UserDTO userDTO, HttpServletResponse response) {
    ...
    String accessToken = jwtUtil.createJwt(userDetails.getUsername(),
        userDetails.getAuthorities().stream().findFirst().get().getAuthority(), 15 * 60 * 1000L); // ✅ Access Token
                                                                                                  // (15분)

    String refreshToken = jwtUtil.createRefreshToken(userDetails.getUsername(), 7 * 24 * 60 * 60 * 1000L); // ✅ Refresh
                                                                                                           // Token (7일)
