...
public class ProductController {
    ...
    @GetMapping("/list")
    public ResponseEntity<?> getFilteredList(@RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size, @RequestParam(required = false) String providerType) {
        try {
    ...
    @GetMapping("/listMap")
    public ResponseEntity<List<ProductResponse>> findAllWithPosition(
            @RequestParam(defaultValue = "37.563588758399376") double swLat,
    ...
    public ResponseEntity<List<String>> getProductTags(@PathVariable Long productId) {
        return ResponseEntity.ok(productService.findById(productId, "internal").getTags());
    }
}
