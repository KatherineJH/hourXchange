...
public class ServiceProductController {
  ...
  public ResponseEntity<?> save(@RequestBody ServiceProductDTO serviceProductDTO) {
    ...
    Map<String, String> categoryCheck =
        categoryService.existsById(serviceProductDTO.getCategoryId());
    ...
    for (int i = 0; i < serviceProductDTO.getImages().size(); i++) {
      Map<String, String> existsByImgUrlCheck =
          spImageServiceImpl.existsByImgUrl(serviceProductDTO.getImages().get(i));
    ...
    Map<String, String> ProviderTypeCheck =
        ProviderType.existsByValue(serviceProductDTO.getProviderType());
