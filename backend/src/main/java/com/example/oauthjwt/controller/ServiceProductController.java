...
public class ServiceProductController {
  ...
  public ResponseEntity<?> save(@RequestBody ServiceProductReqDTO serviceProductReqDTO) {
    ...
    Map<String, String> categoryCheck =
        categoryService.existsById(serviceProductReqDTO.getCategoryId());
    ...
    for (int i = 0; i < serviceProductReqDTO.getImages().size(); i++) {
      Map<String, String> existsByImgUrlCheck =
          spImageServiceImpl.existsByImgUrl(serviceProductReqDTO.getImages().get(i));
    ...
    Map<String, String> ProviderTypeCheck =
        ProviderType.existsByValue(serviceProductReqDTO.getProviderType());
