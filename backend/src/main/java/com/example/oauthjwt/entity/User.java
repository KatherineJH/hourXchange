...
public class User {
  ...
  public static User of(UserRequest userRequest, Address address) {
    return User.builder().email(userRequest.getEmail()).password(userRequest.getPassword()).name(userRequest.getName())
        .username(userRequest.getUsername()).birthdate(userRequest.getBirthdate()).address(address)
