...
class ProductServiceTest {
    ...
    void saveProduct_Success() {
        // given
        ProductRequest request = ProductRequest.builder().title("Test Product").providerType("SELLER").categoryId(1L)
                .address(new AddressRequest("12345", "Main Street", null, null)).images(Collections.emptyList())
                .tags(Collections.emptyList()).lat("37.1234").lng("127.5678").build();
        ...
        Product dummyProduct = Product.builder().id(1L).title("Test Product").hours(1)
                .providerType(ProviderType.parseProviderType("SELLER")).address(Address.of(request.getAddress()))
                .owner(testUser).category(testCategory).build();
    ...
    void saveProduct_InvalidUser_ThrowsException() {
        // given
        ProductRequest request = ProductRequest.builder().title("Bad Product").providerType("SELLER").categoryId(1L)
                .address(new com.example.oauthjwt.dto.request.AddressRequest("12345", "Main Street", null, null))
                .lat("37.1234").lng("127.5678").build();

        given(userRepository.findById(anyLong())).willReturn(Optional.empty());
        CustomUserDetails badUser = new CustomUserDetails(User.builder().id(99L).build());

        // when & then
        assertThatThrownBy(() -> productService.save(request, badUser)).isInstanceOf(ResponseStatusException.class)
                .satisfies(e -> {
