...
class DonationServiceTest {
    ...
    private DonationRequest sampleRequest() {
        return DonationRequest.builder().purpose("help").targetAmount(1000).title("t").description("d")
                .startDate(LocalDate.now().minusDays(1)).endDate(LocalDate.now().plusDays(10))
                .images(List.of("u1", "u2")).build();
    }

    @Test
    @DisplayName("createDonation: 정상 생성")
    void createDonation_Success() {
        ...
        given(donationRepository.save(any(Donation.class))).willAnswer(inv -> {
    ...
    @Test
    @DisplayName("createDonation: 이미지 중복 예외")
    void createDonation_DuplicateImage() {
        ...
        assertThatThrownBy(() -> donationService.createDonation(req, cd)).isInstanceOf(ResponseStatusException.class)
                .satisfies(ex -> {
    ...
    void update_Success() {
        ...
        Donation orig = Donation.of(req, new User() {
            {
                setId(3L);
            }
        });
        orig.setId(7L);
        orig.setCurrentAmount(100);
    ...
    void update_NoAuth() {
        ...
        assertThatThrownBy(() -> donationService.update(8L, req, cd)).isInstanceOf(ResponseStatusException.class)
                .satisfies(ex -> {
                    var rse = (ResponseStatusException) ex;
    ...
    @Test
    @DisplayName("getDonation: 첫 방문 시 view++, 캐시 등록")
    void getDonation_First() {
        Donation d = Donation.of(sampleRequest(), new User() {
            {
                setId(5L);
            }
        });
    ...
    void delete_Success() {
        ...
        when(donationHistoryRepository.save(any(DonationHistory.class))).thenAnswer(inv -> inv.getArgument(0));
    ...
    @Test
    @DisplayName("getTopByProgress/ViewCount/Recent")
    void getTops() {
        Donation d = Donation.of(sampleRequest(), new User() {
            {
                setId(9L);
        ...
        when(donationRepository.findTopByProgress(eq(DonationStatus.ONGOING), any(LocalDate.class),
                eq(PageRequest.of(0, 1)))).thenReturn(list);
        when(donationRepository.findTopByViewCount(eq(DonationStatus.ONGOING), any(LocalDate.class),
                eq(PageRequest.of(0, 1)))).thenReturn(list);
        when(donationRepository.findTopByCreatedAt(eq(DonationStatus.ONGOING), any(LocalDate.class),
                eq(PageRequest.of(0, 1)))).thenReturn(list);
