...
class PaymentItemServiceTest {
    ...
    void list_ReturnsDtoList() {
        // given
        PaymentItem item1 = PaymentItem.builder().id(1L).name("ItemA").time(10).price(1000).build();
        PaymentItem item2 = PaymentItem.builder().id(2L).name("ItemB").time(20).price(2000).build();
        ...
        assertThat(result).extracting(PaymentItemResponse::getId).containsExactly(1L, 2L);
        assertThat(result).extracting(PaymentItemResponse::getName).containsExactly("ItemA", "ItemB");
        assertThat(result).extracting(PaymentItemResponse::getTime).containsExactly(10, 20);
        assertThat(result).extracting(PaymentItemResponse::getPrice).containsExactly(1000, 2000);
